...
/obj/machinery/party/musicwriter/attack_hand(mob/user)
	var/dat = ""
	if(writing)
		dat += "Memory scan completed. <br>Writing from scan of [dummy_name] mind... Please Stand By."
	else if(!coin)
...
/obj/machinery/party/musicwriter/Topic(href, href_list)
	if(href_list["write"])
		if(!writing && !dummy && coin)
			icon_state = "writer_on"
			writing = 1
			dummy = usr
			dummy_name = dummy.name
			var/N = sanitize(input("Name of music") as text|null)
			//dummy << "Please stand still while your data is uploading"
			if(N)
