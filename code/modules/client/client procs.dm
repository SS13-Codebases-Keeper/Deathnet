...
/client/proc/GetHighJob()
	...
	if(src.prefs.job_civilian_high)
		...
		switch(src.prefs.job_medsci_high)
			if(RD)
				work_chosen = "Hostess" //"Research Director"
			if(SCIENTIST)
				work_chosen = "Worker" //"Scientist"
			if(CHEMIST)
				work_chosen = "Liaison" //"Chemist"
			if(CMO)
				work_chosen = "Advocate" //"Chemist"
			if(DOCTOR)
				work_chosen = "Practitioner" //"Medical Doctor"
			if(GENETICIST)
				work_chosen = "Psyker" //"Geneticist"
			if(VIROLOGIST)
				work_chosen = "Candyman" //"Virologist"
			if(ROBOTICIST)
				work_chosen = "Gazer" //"Roboticist"
			if(XENOBIOLOGIST)
				work_chosen = "Lieutenant" //"Xenobiologist"
	else if(src.prefs.job_engsec_high)
		switch(src.prefs.job_engsec_high)
			if(CAPTAIN)
				work_chosen = "Minister" //"Captain"
			if(HOS)
				work_chosen = "Authority Chief" //"Head of Security"
			if(WARDEN)
				work_chosen = "Advisor" //"Warden"
			if(DETECTIVE)
				work_chosen = "Informant" //"Detective"
			if(OFFICER)
				work_chosen = "Authorityman" //"Security Officer"
			if(CHIEF)
				work_chosen = "Maintenance Chief" //"Chief Engineer"
			if(ENGINEER)
				work_chosen = "Maintainer" //"Station Engineer"
			if(ATMOSTECH)
				work_chosen = "Operational Apprentice" //"Atmospheric Technician"
			if(AI)
				work_chosen = "Watchman" //"AI"
			if(CYBORG)
				work_chosen = "Cyborg"
				...
				//deathnet jobs below
				//this does not work as deathnet job bitflags are fucked
				//much like character setup displays, they rely on base ss13
				//fuck lunabay
				// -pattern



	else if(src.prefs.job_auth_high)
		switch(src.prefs.job_auth_high)
			if(MINISTER)
				work_chosen = "Minister"
			if(AUTHCHF)
				work_chosen = "Authority Chief"
			if(ADVISOR)
				work_chosen = "Advisor"
			if(INFORMANT)
				work_chosen = "Informant"
			if(AUTHMAN)
				work_chosen = "Authorityman"
			if(MAINTCHF)
				work_chosen = "Maintenance Chief"
			if(MAINTAINER)
				work_chosen = "Maintainer"
			if(MAINTAPP)
				work_chosen = "Operational Apprentice"
			if(WATCHMAN)
				work_chosen = "Watchman"

	else if(src.prefs.job_citizen_high)
		switch(src.prefs.job_citizen_high)
			if(AMUSER)
				work_chosen = "Hostess"
			if(LTNT)
				work_chosen = "Lieutenant"
			if(WORKER)
				work_chosen = "Worker"
			if(LIAISON)
				work_chosen = "Liaison"
			if(ADVOCATE)
				work_chosen = "Advocate"
			if(PRACTITIONER)
				work_chosen = "Practitioner"
			if(PSYKER)
				work_chosen = "Psyker"
			if(CANDYMAN)
				work_chosen = "Candyman"
			if(GAZER)
				work_chosen = "Gazer"
	else
		work_chosen = "Worker"
