...
datum/preferences
	...
	var/job_citizen_high = 0
	var/job_citizen_med = 0
	var/job_citizen_low = 0
	...
	//DEATHNET PREFS
	var/flaw = "Good-Natured"
	var/addiction = "Substance Abuser"
	var/desire = "Glory"
	var/fear = "Betrayal"
	var/prey = "Worm"
...
/datum/preferences
	...
	proc/ShowChoices(mob/user)
		...
		var/dat = "<html style='-ms-overflow-style:none;'><head><title>DEATHNET INTERLINK</title>"
		dat += "<style type='text/css'>html {overflow: auto;};"
		dat += "body {"
		dat += "overflow:auto; overflow-x:hidden"
		dat += "font-family: 'Arial Black', sans-serif;"
		dat += "font-size: 17px;"
		dat += "background-repeat:repeat-x;"
		dat += "border: 10px ridge #2f303d;"
		dat += "}"
		dat += ".title-bar {"
		dat += "background-color: #2f303d;"
		dat += "color: white;"
		dat += "padding: 10px;"
		...
		dat += "</style>"
		dat += "</head>"
		dat += "<body bgcolor='#000000' text='#c9c9c9' alink='#333553' vlink='#533333' link='#533333'>"
		dat +="<div class='title-bar'>DEATHNET INTERLINK</div>"
		dat += "<p align ='right'>"
																								//?_src_=prefs;preference=name;task=input
		dat += "<script language='javascript'>function deathshit(i) {window.location.assign('?_src_=prefs;deathnet='+i.name+';value='+i.value)}</script>"
		...
		dat += "(Always Random Name: <a href='?_src_=prefs;preference=name'>[be_random_name ? "Yes" : "No"]</a>)"
		dat += "<br>"
...
//		dat += "Secondary Language:<br><a href='byond://?src=\ref[user];preference=language;task=input'>[language]</a><br>"
	...
	//	dat += "Internal Organs: <a href='byond://?src=\ref[user];preference=organs;task=input'>Adjust</a><br>"
		...
		dat += "<b>Flaw:</b> <br>"
		dat += "<select name='flaw' onChange='deathshit(this);'>"
		dat += "<option id='flaw' value='missing_eye' [isdeathy(flaw,"Failed Psyker")?"selected":""]>Failed Psyker</option>"
		dat += "<option id='flaw' value='missing_leg' [isdeathy(flaw,"Industrial Organ")?"selected":""]>Industrial Organ</option>"
		dat += "<option id='flaw' value='bad_natured' [isdeathy(flaw,"Bad-Natured")?"selected":""]>Bad-Natured</option>"
		dat += "<option id='flaw' value='good_natured' [isdeathy(flaw,"Good-Natured")?"selected":""]>Good-Natured</option>"
		dat += "</select><br>"

		dat += "<b>Addiction:</b> <br>"
		dat += "<select name='addiction' onChange='deathshit(this);'>"
		dat += "<option id='addiction' value='substance' [isdeathy(addiction,"Substance Abuser")?"selected":""]>Substance Abuser</option>"
		dat += "<option id='addiction' value='pppleaser' [isdeathy(addiction,"People Pleaser")?"selected":""]>People Pleaser</option>"
		dat += "<option id='addiction' value='pain' [isdeathy(addiction,"Agony")?"selected":""]>Agony</option>"
		dat += "</select><br>"

		dat += "<b>Desire:</b> <br>"
		dat += "<select name='desire' onChange='deathshit(this);'>"
		dat += "<option id='desire' value='glory' [isdeathy(desire,"Glory")?"selected":""]>Glory</option>"
		dat += "<option id='desire' value='riches' [isdeathy(desire,"Riches")?"selected":""]>Riches</option>"
		dat += "<option id='desire' value='power' [isdeathy(desire,"Power")?"selected":""]>Power</option>"
		dat += "<option id='desire' value='safety' [isdeathy(desire,"Safety")?"selected":""]>Safety</option>"
		dat += "<option id='desire' value='danger' [isdeathy(desire,"Danger")?"selected":""]>Danger</option>"
		dat += "</select><br>"

		dat += "<b>Fear:</b> <br>"
		dat += "<select name='fear' onChange='deathshit(this);'>"
		dat += "<option id='fear' value='betrayal' [isdeathy(fear,"Betrayal")?"selected":""]>Betrayal</option>"
		dat += "<option id='fear' value='untruth' [isdeathy(fear,"Untruth")?"selected":""]>Untruth</option>"
		dat += "<option id='fear' value='authority' [isdeathy(fear,"Authority")?"selected":""]>Authority</option>"
		dat += "<option id='fear' value='power' [isdeathy(fear,"Power")?"selected":""]>Power</option>"
		dat += "</select><br>"

		dat += "<b>Prey:</b> <br>"
		dat += "<input type='radio' name='prey' value='bird' [isdeathy(prey,"Bird")?"checked":""] onChange='deathshit(this);'>Bird<br>"
		dat += "<input type='radio' name='prey' value='worm' [isdeathy(prey,"Worm")?"checked":""] onChange='deathshit(this);'>Worm<br>"
...
*/
		...
		user <<browse(dat,"window=player_panel;size=800x800;can_close=1;can_resize = 0;border=0;titlebar=1")
		//user << browse(dat, "window=preferences;size=560x580")

	proc/SetChoices(mob/user, limit = 18, list/splitJobs = list("Chief Medical Officer"), width = 350, height = 550)
		if(!job_master)
			return
		...
		var/HTML = "<body style='color: white; background-color: black;'>"
		HTML += "<tt><center>"
		HTML += "<b>Choose your role.</b><br>Failure is familiar to the inhabitants of CS-5. Be brave today and forget your worries.<br><br>"
		...
		for(var/datum/job/job in job_master.occupations)
			...
			if(jobban_isbanned(user, rank))
				HTML += "<font color=#A61528>[rank]</font></td><td><font color=#A61528><b> \[RESTRICTED]</b></font></td></tr>"
				continue
			...
			if(GetJobDepartment(job, 1) & job.flag)
				HTML += " <font color=#002BFE>\[High]</font>"
			else if(GetJobDepartment(job, 2) & job.flag)
				HTML += " <font color=#19209D>\[Medium]</font>"
			else if(GetJobDepartment(job, 3) & job.flag)
				HTML += " <font color=#2B2050>\[Low]</font>"
			else
	...
	proc/SetJobDepartment(var/datum/job/job, var/level)
		...
		switch(level)
			...
			if(2)//Set current highs to med, then reset them
				...
				job_auth_med |= job_medsci_high
				job_citizen_med |= job_engsec_high
				job_civilian_high = 0
				job_medsci_high = 0
				job_engsec_high = 0
				job_auth_high = 0
		...
		switch(job.department_flag)
			...
			if(AUTH)
				switch(level)
					if(2)
						job_auth_high = job.flag
						job_auth_med &= ~job.flag
					if(3)
						job_auth_med |= job.flag
						job_auth_low &= ~job.flag
					else
						job_auth_low |= job.flag
			if(CITIZEN)
				switch(level)
					if(2)
						job_citizen_high = job.flag
						job_citizen_med &= ~job.flag
					if(3)
						job_citizen_med |= job.flag
						job_citizen_low &= ~job.flag
					else
						job_citizen_low |= job.flag
		return 1
	...
	proc/process_link(mob/user, list/href_list)
		...
		if(!href_list["deathnet"] || href_list["deathnet"] == "prey")
			var/sound/S = sound("sound/deathnet/menu/beep[rand(1,6)].ogg", repeat = 0, wait = 0, volume = 50, channel = 0)
  ...
  // I don't need this bay12 shit
		...
		switch(href_list["deathnet"])
			if("flaw")
				var/sound/S = sound("sound/deathnet/menu/commie2.ogg", repeat = 0, wait = 0, volume = 50, channel = 0)
				S.frequency = get_rand_frequency()
				user << S
				switch(href_list["value"])
					if("missing_eye")
						user << "You once sought to expand upon your mind. You failed.<br>"
						flaw = "Failed Psyker"
					if("missing_leg")
						user << "Born dysfunctional or turned so, one of your organs is dead weight.<br>"
						flaw = "Industrial Organ"
					if("bad_natured")
						user << "Short temper, condescending, vindictive. Standard traits of a slum-runner. And not good ones, either.<br>"
						flaw = "Bad-Natured"
					if("good_natured")
						user << "As a child, you dreamt of peace. As an adult, you dream of a better life. Your optimism carries you, and those who use you, forward.<br>"
						flaw = "Good-Natured"
			...
			if("addiction")
				...
				switch(href_list["value"])
					if("substance")
						user << "Cigarette butts off the floor, powder left on a toilet, and a syringe you found in the trash. Everything should go into your bloodstream.<br>"
						addiction = "Substance Abuser"
					if("pppleaser")
						user << "Companionship is something everyone craves. You however, <i>need</i> it.<br>"
						addiction = "People Pleaser"
					if("pain")
						var/blurb
						switch(gender)
							if(MALE)
							...
							if(FEMALE)
								...
								blurb = "person"
						user << "Sweet suffering is all a [blurb] could dream of in this torterous existence. The only thing keeping you alive is masochism.<br>"
						addiction = "Agony"

			if("desire")
				var/sound/S = sound("sound/deathnet/menu/commie1.ogg", repeat = 0, wait = 0, volume = 50, channel = 0)
				S.frequency = get_rand_frequency()
				user << S
				switch(href_list["value"])
					if("glory")
						user << "You want to do something bold-- All eyes are on you now!<br>"
						desire = "Glory"
					if("riches")
						user << "You need more <b>Rontz.</b> You could <b>never</b> have enough.<br>"
						desire = "Riches"
					if("power")
						user << "You need subordinates. What are you, if not a leader?<br>"
						desire = "Power"
					if("safety")
						user << "Your home is sanctuary. Your well-being is above all. Hurtful souls seek you, and spiteful eyes track you. <b>Survive.</b><br>"
						desire = "Safety"
					if("danger")
						user << "Who cares! They are not important, you are not important! Live every breath like it's your last! It's time to die!<br>"
						desire = "Danger"
			...
			if("fear")
				...
				switch(href_list["value"])
					if("betrayal")
						user << "Turn your back to no-one. Relations are meaningless. Do not let them get the better of you.<br>"
						fear = "Betrayal"
					if("untruth")
						user << "You fucking <b>HATE</b> lies.<br>"
						fear = "Untruth"
					if("authority")
						user << "Fucking authoritymen. Who are they to tell you what to do? Can you even trust them to defend you?<br>"
						if(job_engsec_high || job_auth_high)
							user << "<b>Can you even trust yourself?</b><br>"
						fear = "Authority"
					if("power")
						user << "Who is the Minister? The Dict-Tyrant? Are these real people? You won't buy it until you see their blood yourself.<br>"
						fear = "Power"
	...
	proc/copy_to(mob/living/carbon/human/character, safety = 0)
		...
		switch(flaw)
			if("Failed Psyker")
				var/datum/organ/internal/brain = character.internal_organs_by_name[pick("brain")]
				brain.damage += 14


			if("Assisted Organ")
				var/datum/organ/internal/random_organ = character.internal_organs_by_name[pick("liver", "lungs", "heart", "kidneys")]
				random_organ.mechanize()
				random_organ.damage += 9

//		character.dn_addiction = addiction
//		character.dn_addiction_timer = world.time + 9000 + rand(-1500,1500)
...
datum/preferences/proc/isdeathy(var/target, var/shittotest)
	if(target == shittotest) return 1
	else return 0