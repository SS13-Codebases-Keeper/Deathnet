...
/datum/admins/proc/Secrets()
	...
	if(check_rights(R_FUN,0))
		dat += {"
			...
			<A href='?src=\ref[src];secretsfun=dummy'>Make all players dumb</A><BR>
...
/datum/admins/proc/announce()
	...
	set name = "INTERPLANET DOCTRINE"
	set desc="INTERPLANET DOCTRINE"
	if(!check_rights(0))	return
	...
	if(message)
		...
		world << "\red <b> INTERPLANET DOCTRINE: </b>\n [message]\n "
		world << 'sound/deathnet/jingles/doctrine.ogg'
		log_admin("Announce: [key_name(usr)] : [message]")

/datum/admins/proc/roundendtime()
	set category = "Special Verbs"
	set name = "END OF THE WORLD"
	set desc="IT'S OVER"
	if(!check_rights(0))	return

	var/message2 = sanitize_uni(input("The server will reboot a few ticks after this sends, so make sure it's some good shit.", "ROUND END MESSAGE", null, null) as message)
	if(message2)
		if(!check_rights(R_SERVER,0))
			message2 = sanitize(message2)
		world << "\red [message2]\n "
		world << 'sound/deathnet/jingles/roundover.ogg'
		log_admin("Announce: [key_name(usr)] : [message2]")
		sleep(10)
		world.Reboot()
...
/datum/admins/proc/toggleoocdead()
	set category = "Server"
	set desc="Let us hear your wonderful voice."
...
/datum/admins/proc/startnow()
	...
	if(ticker.current_state == GAME_STATE_PREGAME)
		...
		message_admins("<font color='darkslateblue'>[usr.key] has started the game.</font>")
		return 1
	else
