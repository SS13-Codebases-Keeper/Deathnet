...
var/list/technocrat_roles = list(
	"Advisor",
	"Authorityman",
...
/proc/jobban_isbanned(mob/M, rank)
	if(M && rank)
		...
		*/
		if (rank in technocrat_roles)
			if (!is_dn_technocrat(M.client))
				return "RESTRICTED CLEARANCE"
		if (rank in veteran_roles)
			if (!is_dn_whitelisted(M.client))
				return "RESTRICTED CLEARANCE"
