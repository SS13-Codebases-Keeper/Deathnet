...
/obj/structure/fire_source
	...
	icon = 'icons/obj/firestructures.dmi'
	icon_state = "campfire"
	anchored = 1
	density = 0
...
/obj/structure/fire_source/hobo
	name = "barrel"
	desc = "So homely!"
	icon_state = "hobo"
	open_flame = 0
...
/obj/structure/fire_source/hobo/lit
	fuel = 50                   // How much fuel is left?
	lit = 1

/obj/structure/fire_source/hobob
...
/obj/structure/fire_source/hobob/lit
	fuel = 50                   // How much fuel is left?
	lit = 1

/obj/structure/fire_source/hoboc
	icon = 'icons/obj/64x64.dmi'
	name = "barrel"
	desc = "So homely!"
	icon_state = "hoboc"
...
/obj/structure/fire_source/hoboc/lit
	fuel = 50                   // How much fuel is left?
	lit = 1
...
/obj/structure/fire_source/attackby(var/obj/item/thing, var/mob/user)
	...
	if(istype(thing, /obj/item/weapon/flame) && lit != FIRE_LIT)
...
/obj/structure/fire_source/proc/light()
	...
	playsound(src, fire_sounds, 100, 1)
...
/obj/structure/fire_source/proc/process_fuel()
	...
	for(var/obj/item/thing in contents)
		...
		if(istype(thing, /obj/item/stack/sheet/wood))
			fuel += rand(100,150)
			qdel(thing)
			return 1
	...
	switch(lit)
		if(FIRE_LIT)
			if((fuel > 10) || contents.len)
				...
				playsound(src, fire_sounds, 100, 1)
				src.SetLuminosity(8, 8, 8)
			else if(fuel <= 5)
				need_overlay = "[icon_state]_lit_dying"
				playsound(src, fire_sounds, 100, 1)
				src.SetLuminosity(6, 6, 6)
			else if(fuel <= 10)
				need_overlay = "[icon_state]_lit_low"
				playsound(src, fire_sounds, 100, 1)
				src.SetLuminosity(3, 3, 3)
		if(FIRE_DEAD)
