...
/obj/item/ammo_magazine
	...
	attackby(var/obj/item/A as obj, mob/user as mob)
		...
		if(num_loaded)
			if(num_loaded > 1)
				user << "<span class='notice'>You load [num_loaded] cartridge\s into \the [src]!</span>"
			else
				user << "<span class='notice'>You load a cartridge into \the [src]!</span>"
			update_icon()
	...
	attack_self(mob/user as mob)
		...
		if(A)
			A.loc = user.loc
			user << "<span class='notice'>You remove a cartridge from \the [src]!</span>"
			update_icon()
