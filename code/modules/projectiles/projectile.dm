...
/obj/item/projectile
	...
	var/range = 0
	var/proj_hit = 0
	var/matrix/effect_transform			// matrix to rotate and scale projectile effects
	...
	proc/muzzle_effect(var/matrix/T)
		if(silenced)
			return
		...
		if(ispath(muzzle_type))
			...
			if(istype(M))
				M.set_transform(T)
				current = locate(min(max(x + xo, 1), world.maxx), min(max(y + yo, 1), world.maxy), z)
				M.pixel_x = round(pixel_x, 1)
				M.pixel_y = round(pixel_y, 1)
