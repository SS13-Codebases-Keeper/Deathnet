...
/obj/item/weapon/gun/proc/Fire(atom/target as mob|obj|turf|area, mob/living/user as mob|obj, params, reflex = 0)//TODO: go over this
	...
	src.loc.AddLuminosity(2,2,2)

	user.overlay_fullscreen("supress",/obj/screen/fullscreen/oxy, 6)
...
/obj/item/weapon/gun/attack(mob/living/M as mob, mob/living/user as mob, def_zone)
	...
	if (M == user && user.zone_sel.selecting == "mouth" && !mouthshoot)
		...
		if (process_chambered())
			user.visible_message("<span class = 'warning'>[user] sticks [src] in their mouth and pulls the trigger.</span>")
			if(silenced)
				playsound(user, fire_sound, 10, 1)
			else
				playsound(user, fire_sound, 50, 1)
			...
			M.visible_message("\red [user] sticks [src] in their mouth and pulls the trigger. <b>But their gun clicks.</b>")
			click_empty(user)
			mouthshoot = 0
			return
