...
// DEATHNET AUTH PREFERENCE PREVIEW BELOW

// NOTE TO FUTURE CODERS
//
// The bitflags for jobs are fucked so I am simply commenting out base SS13 loadouts and replacing
// them with deathnet equipment. This can be fixed later down development and nothing is deleted.
// 
// This means that the stuff you see immediately down below is just a dud and previews are operating
// on vanilla SS13 jobs.
//
// I am sorry. -patternseeker
// P.S. ctrl+f "Functionally" on this .dm


		else if(job_auth_high)				
			switch(job_auth_high)
				if(MINISTER)
					clothes_s = new /icon(uniform_icon, "minister_s")
					clothes_s.Blend(new /icon(shoes_icon, "jackboots"), ICON_UNDERLAY)
				if(ADVISOR)
					clothes_s = new /icon(uniform_icon, "advisor_s")
					clothes_s.Blend(new /icon(shoes_icon, "jackboots"), ICON_UNDERLAY)
				if(AUTHCHF)
					clothes_s = new /icon(uniform_icon, "authchf_s")
					clothes_s.Blend(new /icon(shoes_icon, "jackboots"), ICON_UNDERLAY)
				if(INFORMANT)
					clothes_s = new /icon(uniform_icon, "informant_s")
					clothes_s.Blend(new /icon(shoes_icon, "jackboots"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon(gloves_icon, "bgloves"), ICON_UNDERLAY)
				if(WATCHMAN)
					clothes_s = new /icon(uniform_icon, "watchman_s")
					clothes_s.Blend(new /icon(shoes_icon, "jackboots"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon(gloves_icon, "watchman"), ICON_UNDERLAY)
				if(AUTHMAN)
					clothes_s = new /icon(uniform_icon, "authman_s")
					clothes_s.Blend(new /icon(shoes_icon, "authman"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon(suit_icon, "authman"), ICON_OVERLAY)
				if(MAINTCHF)
					clothes_s = new /icon(uniform_icon, "maintainer_s")
					clothes_s.Blend(new /icon(shoes_icon, "maintainer"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon(gloves_icon, "maintainer"), ICON_UNDERLAY)
				if(MAINTAINER)
					clothes_s = new /icon(uniform_icon, "maintainer_s")
					clothes_s.Blend(new /icon(shoes_icon, "maintainer"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon(shoes_icon, "maintainer"), ICON_UNDERLAY)
				if(MAINTAPP)
					clothes_s = new /icon(uniform_icon, "maintapp_s")
					clothes_s.Blend(new /icon(shoes_icon, "maintapp"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon(gloves_icon, "maintapp"), ICON_UNDERLAY)

			//DEATHNET CITIZEN PREFERENCE PREVIEW BELOW
		else if(job_citizen_high)				
			switch(job_citizen_high)
				if(AMUSER)
					clothes_s = new /icon(uniform_icon, "amuser_s")
					clothes_s.Blend(new /icon(shoes_icon, "jackboots"), ICON_UNDERLAY)
				if(LTNT)
					clothes_s = new /icon(uniform_icon, "ltnt_s")
					clothes_s.Blend(new /icon(shoes_icon, "jackboots"), ICON_UNDERLAY)
				if(WORKER)
					clothes_s = new /icon(uniform_icon, "worker_s")
					clothes_s.Blend(new /icon(shoes_icon, "worker"), ICON_UNDERLAY)
				if(LIAISON)
					clothes_s = new /icon(uniform_icon, "liaison_s")
					clothes_s.Blend(new /icon(shoes_icon, "centcom"), ICON_UNDERLAY)
				if(ADVOCATE)
					clothes_s = new /icon(uniform_icon, "advocate_s")
					clothes_s.Blend(new /icon(shoes_icon, "centcom"), ICON_UNDERLAY)
				if(PRACTITIONER)
					clothes_s = new /icon(uniform_icon, "practitioner_s")
					clothes_s.Blend(new /icon(shoes_icon, "jackboots"), ICON_UNDERLAY)
				if(PSYKER)
					clothes_s = new /icon(uniform_icon, "psyker_s")
					clothes_s.Blend(new /icon(shoes_icon, "worker"), ICON_UNDERLAY)
				if(CANDYMAN)
					clothes_s = new /icon(uniform_icon, "candyman_s")
					clothes_s.Blend(new /icon(shoes_icon, "centcom"), ICON_UNDERLAY)
				if(GAZER)
					clothes_s = new /icon(uniform_icon, "gazer_s")
					clothes_s.Blend(new /icon(shoes_icon, "gazer"), ICON_UNDERLAY)
			...
			switch(job_medsci_high)
				if(RD) //Functionally amuser
					clothes_s = new /icon(uniform_icon, "amuser_s")
					clothes_s.Blend(new /icon(shoes_icon, "amuser"), ICON_UNDERLAY)
					...
					switch(backbag)
						...
						if(4)
							clothes_s.Blend(new /icon(back_icon, "satchel"), ICON_OVERLAY)*/
				if(SCIENTIST) //Functionally worker
					clothes_s = new /icon(uniform_icon, "worker_s")
					clothes_s.Blend(new /icon(shoes_icon, "worker"), ICON_UNDERLAY)
					...
					switch(backbag)
						...
						if(4)
							clothes_s.Blend(new /icon(back_icon, "satchel"), ICON_OVERLAY)*/
				if(CHEMIST) //Functionally liaison
					clothes_s = new /icon(uniform_icon, "liaison_s")
					clothes_s.Blend(new /icon(shoes_icon, "centcom"), ICON_UNDERLAY)
				...
				if(CMO) //Functionally advocate
					...
					switch(backbag)
						...
						if(4)
							clothes_s.Blend(new /icon(back_icon, "satchel"), ICON_OVERLAY)*/
				if(DOCTOR) //Functionally practitioner
					clothes_s = new /icon(uniform_icon, "practitioner_s")
					clothes_s.Blend(new /icon(shoes_icon, "jackboots"), ICON_UNDERLAY)
					...
					switch(backbag)
						...
						if(4)
							clothes_s.Blend(new /icon(back_icon, "satchel"), ICON_OVERLAY)*/
				if(GENETICIST) //Functionally psyker
					clothes_s = new /icon(uniform_icon, "psyker_s")
					clothes_s.Blend(new /icon(shoes_icon, "worker"), ICON_UNDERLAY)
					...
					switch(backbag)
						...
						if(4)
							clothes_s.Blend(new /icon(back_icon, "satchel"), ICON_OVERLAY)*/
				if(VIROLOGIST) //Functionally candyman
					clothes_s = new /icon(uniform_icon, "candyman_s")
					clothes_s.Blend(new /icon(shoes_icon, "centcom"), ICON_UNDERLAY)
					...
					switch(backbag)
						...
						if(4)
							clothes_s.Blend(new /icon(back_icon, "satchel"), ICON_OVERLAY)*/
				if(ROBOTICIST) //Functionally gazer
					clothes_s = new /icon(uniform_icon, "gazer_s")
					clothes_s.Blend(new /icon('icons/mob/mask.dmi', "gazer"), ICON_OVERLAY)
					...
					switch(backbag)
						...
						if(4)
							clothes_s.Blend(new /icon(back_icon, "satchel"), ICON_OVERLAY)*/
				if(XENOBIOLOGIST) //Functionally lieutenant
					clothes_s = new /icon(uniform_icon, "ltnt_s")
					clothes_s.Blend(new /icon(shoes_icon, "centcom"), ICON_UNDERLAY)
					/*clothes_s = new /icon(uniform_icon, "robotics_s")
					clothes_s.Blend(new /icon(shoes_icon, "black"), ICON_UNDERLAY)
			...
			switch(job_engsec_high)
				if(CAPTAIN) //Functionally minister
					clothes_s = new /icon(uniform_icon, "minister_s")
					clothes_s.Blend(new /icon(shoes_icon, "jackboots"), ICON_UNDERLAY)
					...
					switch(backbag)
						...
						if(4)
							clothes_s.Blend(new /icon(back_icon, "satchel"), ICON_OVERLAY)*/
				if(HOS) //Functionally authoritychief
					clothes_s = new /icon(uniform_icon, "authchf_s")
					clothes_s.Blend(new /icon(shoes_icon, "jackboots"), ICON_UNDERLAY)
					...
					switch(backbag)
						...
						if(4)
							clothes_s.Blend(new /icon(back_icon, "satchel"), ICON_OVERLAY)*/
				if(WARDEN) //Functionally advisor
					clothes_s = new /icon(uniform_icon, "advisor_s")
					clothes_s.Blend(new /icon(shoes_icon, "jackboots"), ICON_UNDERLAY)
					...
					switch(backbag)
						...
						if(4)
							clothes_s.Blend(new /icon(back_icon, "satchel"), ICON_OVERLAY)*/
				if(DETECTIVE) //Functionally informant
					clothes_s = new /icon(uniform_icon, "informant_s")
					clothes_s.Blend(new /icon(shoes_icon, "jackboots"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon(gloves_icon, "bgloves"), ICON_UNDERLAY)
					...
					switch(backbag)
						...
						if(4)
							clothes_s.Blend(new /icon(back_icon, "satchel"), ICON_OVERLAY)*/
				if(OFFICER) //Functionally authorityman
					clothes_s = new /icon(uniform_icon, "authman_s")
					clothes_s.Blend(new /icon(shoes_icon, "authman"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon(suit_icon, "authman"), ICON_OVERLAY)
					...
					switch(backbag)
						...
						if(4)
							clothes_s.Blend(new /icon(back_icon, "satchel"), ICON_OVERLAY)*/
				if(CHIEF) //Functionally maintenance chief
					clothes_s = new /icon(uniform_icon, "maintainer_s")
					clothes_s.Blend(new /icon(shoes_icon, "maintainer"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon(gloves_icon, "maintainer"), ICON_UNDERLAY)
					...
					switch(backbag)
						...
						if(4)
							clothes_s.Blend(new /icon(back_icon, "satchel"), ICON_OVERLAY)*/
				if(ENGINEER) //Functionally maintainer
					clothes_s = new /icon(uniform_icon, "maintainer_s")
					clothes_s.Blend(new /icon(shoes_icon, "maintainer"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon(gloves_icon, "maintainer"), ICON_UNDERLAY)
					...
					switch(backbag)
						...
						if(4)
							clothes_s.Blend(new /icon(back_icon, "satchel"), ICON_OVERLAY)*/
				if(ATMOSTECH) //Functionally operational apprentice
					clothes_s = new /icon(uniform_icon, "maintapp_s")
					clothes_s.Blend(new /icon(shoes_icon, "maintapp"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon(gloves_icon, "maintapp"), ICON_UNDERLAY)
				...
				if(AI) //Functionally watchman. Don't even question it.
					clothes_s = new /icon(uniform_icon, "watchman_s")
					clothes_s.Blend(new /icon(shoes_icon, "jackboots"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon(gloves_icon, "watchman"), ICON_UNDERLAY)
					/*clothes_s = new /icon(uniform_icon, "grey_s")
