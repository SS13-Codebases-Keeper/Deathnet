...
mob/proc/sendtoclone() //If the coordinates for T2 are still 27, 12, 1 then the mappers are lazy fucking bastards
	var/list/genderzlol = list("male", "female")
	var/list/welcomeback = list (" I live ... again.", " Feels good to be back.", " Mmm ...", " I feel lethargic. Gotta get moving.", " I wonder who pays for this facility.", " Hrm ... what was my name, again?")
	var/list/clonelocation = list(locate(65, 63, 9), locate (64, 63, 9))
	var/mob/living/carbon/human/new_body = new(pick(clonelocation))
	new_body.real_name = "Clone" + " #[rand(1,1000)]"
	new_body.gender = pick(genderzlol)
	new_body.update_hair()
	new_body.update_body()
	src.mind.transfer_to(new_body)
	src.clearHUD()
	new_body << 'sound/deathnet/jingles/respawn.ogg'
	new_body << "<B> You're spat out of the all too friendly and familiar pod.</B>"
	sleep(30)
	new_body << pick(welcomeback)
	sleep(30)
	new_body << "\red <B> You vaguely recall a terrible place. The details elude you.</B>"
	return

/mob/verb/abandon_mob()
	set name = "Reshape" //Neon what the fuck is wrong with you
	set category = "OOC"

	if(is_dn_whitelisted(client))
		sendtoclone() //handles sending people to the Cloning Lab

	if (!( abandon_allowed ))
		usr << "\blue Reshaping is forbidden."
		return
	...
	var/deathtime = world.time - src.timeofsinnerization

	if (deathtime < 2 MINUTES)
		usr << "<B>Not yet.</B>"
		return
	else
		sendtoclone()
	...
	if(spell_list && spell_list.len)
		for(var/obj/effect/proc_holder/spell/S in spell_list)
			switch(S.charge_type)
				if("recharge")
					statpanel("PSY-CONTROL","[S.charge_counter/10.0]/[S.charge_max/10]",S)
				if("charges")
					statpanel("PSY-CONTROL","[S.charge_counter]/[S.charge_max]",S)
				if("holdervar")
					statpanel("PSY-CONTROL","[S.holder_var_type] [S.holder_var_amount]",S)
