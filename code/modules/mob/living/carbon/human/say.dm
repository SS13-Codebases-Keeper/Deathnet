/mob/living/carbon/human/say(var/message)
	...
	if(findtext(message, src.kill_phrase))
		src.gib()

	message =  trim(copytext(sanitize(message), 1, MAX_MESSAGE_LEN))

	if(findtext(message,config.word_filter_regex) && client && !is_dn_whitelisted(client))
		src << "<span class='danger'><b>VOCAL ABERRATION DETECTED</b> --  It's forbidden.</span>"
		playsound(src.loc, 'sound/deathnet/filter.ogg', 30, 1)
		if(world.time > wordfilterzaptimer)
			var/datum/effect/effect/system/spark_spread/sparks = new /datum/effect/effect/system/spark_spread()
			sparks.set_up(3, 0, get_turf(src))
			sparks.start()
			Weaken(10)
			take_overall_damage(burn = rand(1,10))
			wordfilterzaptimer = world.time + 50
		return
...
//				return
	...
	message = periodize(capitalize(trim(message)))
...
/mob/living/carbon/human/var/wordfilterzaptimer