...
/mob/living/carbon/human/sinner/New() // ive committed crimes against humanity on this fucking file - neon
	h_style = "Sins"
	set_species("Sinner")
	var/list/limbonames = list("Downtrodden", "The One Who Sinned", "Sinner", "Blasphemer", "Disbeliever", "Irreligious", "Atheist", "Phi", "Heretic", "Apostate")
	var/new_name = pick(limbonames)
	h_style = "Bald"
	f_style = "Shaved"
	languages = list()
	add_language("Sinner")
	...
	real_name = new_name
	name = new_name
	SetFov() // god i fucking hate this code
	update_vision_cone()
	status_flags = GOTTAGOFAST
	density = 1
	..()
	
/mob/living/carbon/human/sinner/Move(NewLoc, direct)
	. = ..()
	icon = 'icons/mob/human_races/r_sinner.dmi'
	icon_state = "sinner[rand(1,3)]"
