/mob/living/carbon/human/examine()
	...
	var/skipeyes = 0
	var/skipface = 0
	//var/dn_comrade_only = TRUE - Commented out unused var temporarily
	...
	// whitelist check
	if(is_dn_whitelisted(src.client) && is_dn_whitelisted(usr.client))
		msg += "<span class='warning'><b>One of us.</b></span>\n"
	if(is_dn_whitelisted(src.client))
		msg += "<span class='warning'>[kill_phrase] is [t_his] kill phrase.</span>\n"
	...
	if((!species.has_organ["brain"] || has_brain()) && stat != DEAD)
		if(!key)
			msg += "<span class='deadsay'>[t_He] [t_is] lost to the Entropy...</span>\n"
		else if(!client)
