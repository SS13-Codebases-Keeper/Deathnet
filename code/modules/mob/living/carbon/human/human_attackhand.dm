/mob/living/carbon/human/attack_hand(mob/living/carbon/human/M as mob)
	if (istype(loc, /turf) && istype(loc.loc, /area/start))
		...
		return
	/*if (client && M.client) -- 30.05.2023 we got found out. -Patternseeker
		if(!is_dn_whitelisted(M.client) && is_dn_whitelisted(client))
			M << "<span class='danger'><b>INTERACTION DENIED - YOUR ACCESS LEVEL IS NOT HIGH ENOUGH.</b></span>"
			return*/
...
//			if(M.health < -75)	return 0
		...
		if("hurt")
			...
			M.attack_log += text("\[[time_stamp()]\] <font color='red'>[pick(attack.attack_verb)]es [src.name] ([src.ckey])</font>")
			src.attack_log += text("\[[time_stamp()]\] <font color='orange'>Has been [pick(attack.attack_verb)]es by [M.name] ([M.ckey])</font>")
			msg_admin_attack("[key_name(M)] [pick(attack.attack_verb)]es [key_name(src)]")
			...
			dmgTXT += "\red <B>[M]</B> [pick(attack.attack_verb)]es <B>[src]!</B> "
...
/mob/living/carbon/human/sinner/attack_hand(mob/living/carbon/human/sinner/M as mob)
	if (client && M.client)
		switch(M.a_intent)
			if("help")
				M << "\red Your days of harming are over." //GHOST PACIFICATION
				return
			if("disarm")
				M << "\red Your days of harming are over." //GHOST PACIFICATION
				return
