...
/client/Northeast() //Honestly fuck old SS13.
	if(iscarbon(usr))
		var/mob/living/carbon/C = usr
		C.swap_hand()
	else
		usr << "\red This mob type cannot swap hands."
	return
...
/client/Southeast() //yeen getcho
	var/turf/controllerlocation = locate(1, 1, usr.z) //get zcontroller
	if(iscarbon(usr))
		var/mob/living/carbon/C = usr
		C.lay_down()

	/*else if
		usr << "\red This mob type cannot lay down."*/

	//moves ghosts 1z down
	else if (istype(mob, /mob/dead/observer))
		for(var/obj/effect/landmark/zcontroller/controller in controllerlocation)
			if (controller.down)
...
/client/Center()
	/* No 3D movement in 2D spessman game. dir 16 is Z Up - not anymore
	if (isobj(mob.loc))
		var/obj/O = mob.loc
		if (mob.canmove)
...
/client/Move(n, direct)
	...
	if(isturf(mob.loc))
		var/area/target_area = get_area(get_step(mob,direct))
		if (target_area.private_party && mob.client && !is_dn_whitelisted(mob.client))
			mob << "<span class='danger'><b>INTERACTION DENIED - YOUR ACCESS LEVEL IS NOT HIGH ENOUGH.</b></span>"
			return 0
