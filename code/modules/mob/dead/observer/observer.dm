...
/mob/living/verb/ghost()
	set category = "OOC"
	set name = "Relinquish Life"
	set desc = "Go easy, now."
	if(is_dn_whitelisted(client))
		if(stat == DEAD)
			ghostize(1) //Makes you an SS13 ghost
		else
		...
		sinnerize() //Makes you a limbo sinner

mob/proc/sinnerize()
	if (stat == DEAD)
		var/T = locate(27, 12, 1)
		var/mob/living/carbon/human/sinner/new_body = new(T)
		src << 'sound/deathnet/welcometohell.ogg'
		src.mind.transfer_to(new_body)
		src.clearHUD()
		timeofsinnerization = world.time
	else
		usr << "<B>Not yet.</B>"
