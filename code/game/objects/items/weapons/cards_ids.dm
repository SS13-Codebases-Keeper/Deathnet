...
/obj/item/weapon/card/id/proc/update_label(var/newname, var/newjob)
	if(newname || newjob)
		name = "[(!newname)	? "identification card"	: "[newname]'s passport"][(!newjob) ? "" : " ([newjob])"]"
		return

	name = "[(!registered_name)	? "identification card"	: "[registered_name]'s passport"][(!assignment) ? "" : " ([assignment])"]"


/obj/item/weapon/card/id
	name = "identification card"
	desc = "A passport issued to every citizen of the City sectors."
	icon_state = "id"
	item_state = "card-id"
	w_class = 1.0
	var/access = list()
...
/obj/item/weapon/card/id/update_icon()
	..()
	icon_state = "[initial(icon_state)][folded ? "" : "-[folded]"]"
	return

/obj/item/weapon/card/id/attack_self(mob/user as mob)
	folded = !folded
	user << "<span class='notice'>You [folded ? "unfold" : "fold"] the [src].</span>"
	update_icon()


/obj/item/weapon/card/id/examine(mob/user as mob)
	..()
	read()

	/*for(var/mob/O in viewers(user, null))
		user << (text("The passport reads: \icon[] []: assignment: []", user, src, src.name, src.assignment), 1)
...
/obj/item/weapon/card/id/verb/read()
	...
	if(folded)
		/*usr << text("\icon[] []: The passport reads: [].", src, src.name, src.assignment)*/
		usr << "Blood Type: [blood_type]."
		usr << "DNA Hash: [dna_hash]."
		usr << "Fingerprint Hash: [fingerprint_hash]."
		usr << "If found, please return to your nearest Authority mission."
		return
	else
		usr << "\red <b>Unfold the passport first</b>."
		return
...
/obj/item/weapon/card/id/mastermind
	name = "passport (mastermind)"
	icon_state = "id_mastermind"
	item_state = "card_id"

/obj/item/weapon/card/id/enforcer
	name = "passport (enforcer)"
	icon_state = "id_enforcer"
	item_state = "card_id"

/obj/item/weapon/card/id/specialist
	name = "passport (specialist)"
	icon_state = "id_specialist"
	item_state = "card_id"

/obj/item/weapon/card/id/horror
	name = "passport (horrorworker)"
	icon_state = "id_horror"
	item_state = "card_id"

/obj/item/weapon/card/id/citizen
	name = "passport (citizen)"
	icon_state = "id_citizen"
	item_state = "card_id"