...
/atom/proc/examine()
	...
	if (!( usr ))
		return
	usr << "That's \a \icon[src] [src]." //changed to "That's" from "This is" because "This is some metal sheets" sounds dumb compared to "That's some metal sheets" ~Carn
	usr << desc
	// *****RM
...
/atom/proc/add_vomit_floor(mob/living/carbon/M as mob, var/toxvomit = 0)
	if( istype(src, /turf/simulated) )
		...
		if(toxvomit)
			this.icon_state = "vomittox_[pick(1,3)]"
