...
/datum/game_mode/traitor/announce()
	world << "<B>The current game mode is - Hysteria!</B>"
	world << "<B>There are CS-7 operatives in CS-5. Do not let them succeed!</B>"
...
/datum/game_mode/traitor/pre_setup()
	...
	var/num_traitors = num_players() //everyone is a traitor lol
...
/datum/game_mode/proc/forge_traitor_objectives(var/datum/mind/traitor)
	switch(rand(1,100))
		...
		if(13 to 25)
			var/datum/objective/assassinate/stealrontz_objective = new
			stealrontz_objective.owner = traitor
			stealrontz_objective.find_target()
			traitor.objectives += stealrontz_objective
		if(26 to 38)
			var/datum/objective/assassinate/selldrugs_objective = new
			selldrugs_objective.owner = traitor
			selldrugs_objective.find_target()
			traitor.objectives += selldrugs_objective
		if(39 to 51)
			var/datum/objective/assassinate/sabotage_objective = new
			sabotage_objective.owner = traitor
			sabotage_objective.find_target()
			traitor.objectives += sabotage_objective
		if(52 to 64)
			var/datum/objective/assassinate/bombshit_objective = new
			bombshit_objective.owner = traitor
			bombshit_objective.find_target()
			traitor.objectives += bombshit_objective
		if(63 to 75)
			var/datum/objective/assassinate/prosletyze_objective = new
			prosletyze_objective.owner = traitor
			prosletyze_objective.find_target()
			traitor.objectives += prosletyze_objective
		if(76 to 88)
			var/datum/objective/assassinate/usurp_objective = new
			usurp_objective.owner = traitor
			usurp_objective.find_target()
			traitor.objectives += usurp_objective
		else
			var/datum/objective/harm/harm_objective = new
			harm_objective.owner = traitor
			harm_objective.find_target()
			traitor.objectives += harm_objective
	/*switch(rand(1,100))     //when the escape shit is added
		if(1 to 100)
...
/datum/game_mode/proc/greet_traitor(var/datum/mind/traitor)
	traitor.current << "<B><font size=3 color=red>CS-7 operatives surround you.</font></B>"
	var/obj_count = 1
